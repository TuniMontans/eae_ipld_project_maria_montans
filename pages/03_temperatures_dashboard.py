...
min_temp_row = temps_df[temps_df["AvgTemperatureCelsius"] == min_temp]
max_temp_row = temps_df[temps_df["AvgTemperatureCelsius"] == max_temp]

min_temp_city = min_temp_row["City"].iloc[0]
min_temp_date = min_temp_row["Date"].iloc[0]

max_temp_city = max_temp_row["City"].iloc[0]
max_temp_date = max_temp_row["Date"].iloc[0]
...
if unique_cities_list is not None and len(selected_cities) > 0:
    ...
    plt.title(f"Temperatures from {start_date} to {end_date}")   # TODO
    plt.xlabel("Date")  # TODO
    plt.ylabel("Temperature (Celsius)")  # TODO
    ...
    for city in selected_cities:
         city_df = temps_df[temps_df["City"] == city]   # TODO
         city_df_period = city_df[(city_df["Date"] >= start_date) & (city_df["Date"] <= end_date)]  # TODO
         plt.hist(city_df_period["AvgTemperatureCelsius"], bins=20, alpha=0.4, label=city) # TODO

    plt.title(f"Temperatures from {start_date} to {end_date}")   # TODO
    plt.xlabel("Temperature (Celsius)")  # TODO
    plt.ylabel("Freq. (Days)")  # TODO
