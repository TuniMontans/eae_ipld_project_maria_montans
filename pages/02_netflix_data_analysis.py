...
movies_df_no_NaN = movies_df.fillna("Unknown")

country_list = [] 

for country in movies_df_no_NaN["country"]:
    single_countries = country.split(",")
    for each_country in single_countries:     
        country_list.append(each_country.strip())

unique_countries = set(country_list)

if "Unknown" in unique_countries:
...
movies_by_year_df = movies_df.loc[movies_df["release_year"] == year]
top_10_countries = movies_by_year_df["country"].value_counts().head(10)
...
just_movies_df["duration_in_minutes"] = just_movies_df["duration"].apply(lambda x: int(x.split()[0]))

movies_avg_duration_per_year = just_movies_df.groupby("release_year")["duration_in_minutes"].mean()
